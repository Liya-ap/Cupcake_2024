...
public class OrderMapper {
    public static List<Order> getACustomersOrders(int userId, ConnectionPool connectionPool) throws DatabaseException {
        List<Order> listOfAllOrders = new ArrayList<>();

        String sql = """
                SELECT public.order.order_id, bottom.flavor AS bottomflavor, topping.flavor AS toppingflavor, cupcakedetail.price_each AS priceeach, cupcakedetail.amount AS amount, (price_each * amount) AS totalprice\s
                FROM public.order
        ...
        try (
            ...
            while (rs.next()) {
                ...
                listofCupcakes.add(new Cupcake(orderId, new Topping(toppingFlavor), new Bottom(bottomFlavor), priceEach, amount, totalPrice));
    ...
    public static List<Order> getAllOrders(ConnectionPool connectionPool) throws DatabaseException {
        ...
        String sql = """
                SELECT public.order.order_id, users.email, bottom.flavor AS bottomflavor, topping.flavor AS toppingflavor, cupcakedetail.price_each AS priceeach, cupcakedetail.amount AS amount, (price_each * amount) AS totalprice
                FROM public.order
                INNER JOIN public.orderline ON public.order.order_id = orderline.order_id
                INNER JOIN users ON public.order.user_id = users.user_id
                INNER JOIN cupcakedetail ON orderline.cupcakedetail_id = cupcakedetail.cupcakedetail_id
